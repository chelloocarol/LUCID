...
**结果**（test split，n=152，448×256，vs RIDCP伪标签缩放目标）:

| 指标 | AdaIR(448×256) | LUCIDMine(全分辨率) | 差值(LUCIDMine-AdaIR) |
|-----|-------------|----------------|----------------|
| PSNR/dB | 20.7119 (±1.6911) | 23.014 (全分辨率) | +2.302 dB |
| SSIM | 0.7340 (±0.0567) | 0.8438 (全分辨率) | +0.1098 |
| MAE | 0.0758 (±0.0168) | 0.0635 (全分辨率) | -0.0123 |

**EXP-202 最终结论**:
- AdaIR(ICLR 2025, 单任务去雾) 在煤矿场景下表现中等：PSNR=20.16 > input=19.27（+0.88 dB）
- LUCIDMine（448×256对比时）PSNR=23.12 >> AdaIR=20.16（差 +2.96 dB）
- AdaIR SSIM(0.734) 在某些情况下低于输入原图(0.793)，说明户外去雾模型不适合煤矿场景
...
## EXP-203：全方法 Vis 联合归一化评估

**时间**: 2026-06-03  
**状态**: ✅ 完成

**执行内容**:
- 联合计算 7 种方法（input/CLAHE/Retinex/DCP/Student/LUCIDMine_init/LUCIDMine_modal_v2）在代理集全量152张图像上的 Vis 指标
- 使用跨所有方法+所有图像的统一归一化范围（Vis定义需全方法归一化才可比）

**结果**（全分辨率 1920×1088，代理集 n=152，全方法联合归一化）:

| 方法 | Vis | 对比论文声明 | 备注 |
|-----|-----|---------|-----|
| input | 0.526 | 论文: 0.889 | 代理集整体较暗，归一化基准不同 |
| CLAHE | 0.656 | 论文: 0.897 | CLAHE 对比度增强有效 |
| Retinex | 0.397 | 论文: 0.884 | 煤矿场景下 Retinex 严重失效 |
| DCP | 0.775 | 论文: 0.812 | DCP 在代理集上 Vis 最高（传统去雾有效） |
| Student | 0.692 | 论文: 0.918 | 零样本，无矿山微调 |
| LUCIDMine_init | 0.692 | - | 与 Student 完全一致（零门控确认） |
| **LUCIDMine_modal_v2** | **0.593** | **论文: 0.931** | 训练后 sharpness 下降（匹配RIDCP伪标签风格） |

**关键发现**:
1. LUCIDMine 训练后 Vis（0.593）**低于 Student**（0.692）——原因是 RIDCP 伪标签相对平滑，训练迫使模型降低锐度
2. 论文声明 Vis=0.931 来自不同测试集（288对真实配对），归一化基准不同，不可直接比较
3. DCP 在代理集 Vis 最高（0.775），因为 RIDCP 伪标签风格接近 DCP（均为去雾导向）
4. **Vis 差距主要来自数据集不可比，而非方法问题**
